...
const ART={"capital":"assets/capital.webp","Ta":"assets/Ta.webp","Imif":"assets/Imif.webp","Sithriru":"assets/Sithriru.webp","Ana":"assets/Ana.webp","bronze":"assets/bronze.webp","neolithic":"assets/neolithic.webp"};
const DISTRICT_COVERS={capital:'courtyard/cover-capital.webp',bronze:'courtyard/cover-bronze.webp',neolithic:'courtyard/cover-neolithic.webp'};
...
let archive,walkData,portraits={},translatedEvents=[],people=[],preparedPlay,worldView,journey=null,frameIndex=116,activeView='home',routeToken=0,eventLimit=80,peopleLimit=72,timelineTimer=null;
...
function home(){
 $('hero-art').src=DISTRICT_COVERS.capital;
 $('destinations').innerHTML=SCENES.map((s,i)=>`<article class="destination"><button class="destination-image" data-district="${s.id}" aria-label="Enter ${esc(s.name)}"><img src="${esc(DISTRICT_COVERS[s.id])}" alt="In-engine view of the inhabited district of ${esc(s.name)}" loading="lazy"><span class="destination-index">0${i+1}</span><span class="destination-date">${E.year(s.year)}</span></button><div class="eyebrow">${esc(s.era)}</div><h3>${esc(s.name)}</h3><p class="description">${esc(s.description)}</p><a class="destination-link" href="#district/${s.id}">Explore the inhabited district ↗</a><button class="destination-link archive-walk" data-walk="${i}">Earlier archive walk</button></article>`).join('');
...
async function createWorldView(){
 ...
 function update(){const f=d.frames[frameIndex];ownership=owners(f);pols=new Map(f.pol.map(p=>[p.id,p]));ctx.clearRect(0,0,textureCanvas.width,textureCanvas.height);ctx.drawImage(natural,0,0,textureCanvas.width,textureCanvas.height);
  ...
  journey?.paintCities(ctx,f,textureCanvas.width,textureCanvas.height);texture.needsUpdate=true;flatPaint();const selected=$('polity-select').value;$('polity-select').innerHTML='<option value="">Select a polity…</option>'+f.pol.slice().sort((a,b)=>a.n.localeCompare(b.n)).map(p=>`<option value="${p.id}">${esc(p.n)}</option>`).join('');if(selected!==''&&pols.has(Number(selected))){$('polity-select').value=selected;showPolity(Number(selected));}else $('place-detail').innerHTML='<div class="compass-mark">✧</div><h2>A world of places.</h2><p>Select a territory or choose a polity above.</p>';
 ...
 const oldShowPolity=showPolity;showPolity=function(id){oldShowPolity(id);const p=pols.get(Number(id));if(p)journey?.polityLinks(p);};
 function focusCell(x,y){const u=(x+.5)/180,theta=(y+.5)/90*Math.PI,lx=-Math.cos(u*Math.PI*2)*Math.sin(theta),ly=Math.cos(theta),lz=Math.sin(u*Math.PI*2)*Math.sin(theta);angleY=Math.atan2(-lx,lz);angleX=Math.atan2(ly,Math.hypot(lx,lz));zoom=3.2;draw();}
 new ResizeObserver(resize).observe($('planet-stage'));$('map-empty').classList.add('hidden');$('polity-select').onchange=()=>{journey?.clearCity();update();showPolity($('polity-select').value);};
 projection(mode);update();return {update,projection,draw,focusCell,stop:()=>cancelAnimationFrame(raf),showPolity,inspect:()=>({mode,ownership:ownership.length,polities:pols.size,renderer:!!renderer,frame:frameIndex})};
}
...
function renderEvents(){
 ...
 const filtered=translatedEvents.filter(e=>(!journey||journey.acceptEvent(e))&&(!kind||e.k===kind)&&(!$('event-time-filter').checked||e.yr<=year)&&(!term||fold(e.text+' '+(E.kinds[e.k]||e.k)).includes(term)));
...
function renderPeople(){const term=fold($('people-search').value),filtered=people.filter(p=>(!journey||journey.acceptPerson(p))&&(!term||fold(`${p.n} ${p.pn} ${p.r} ${p.era}`).includes(term)));$('people-count').textContent=`${E.number(filtered.length)} notable-person records. Most records have no illustrated portrait.`;
...
async function route(){
 const token=++routeToken,raw=location.hash.slice(1)||'home',parts=raw.split('?')[0].split('/'),params=new URLSearchParams(raw.includes('?')?raw.slice(raw.indexOf('?')+1):''),name=parts[0]==='district'?'courtyard':['home','atlas','chronicle','people','about','walk','courtyard'].includes(parts[0])?parts[0]:'home';
 ...
 try{
  ...
  if(name==='courtyard'){const district=['capital','bronze','neolithic'].includes(parts[1])?parts[1]:'capital';const old=$('courtyard-frame'),next=old.cloneNode(false);next.removeAttribute('src');next.removeAttribute('srcdoc');if(window.TERRA_COURTYARD_HTML)next.srcdoc=window.TERRA_COURTYARD_HTML.replace('<script>','<script>window.DISTRICT_BOOT='+JSON.stringify(district)+';');else next.src=new URL('courtyard/index.html?v=0.6.0&era='+district,document.baseURI).href;old.replaceWith(next);status('');return;}
  ...
  if(token===routeToken){await journey?.afterRoute(name,params);status('');}
 }catch(e){if(token===routeToken)fail(e);}
}
function sendWalk(data){$('walk-frame').contentWindow?.postMessage(data,'*');}
window.TERRA_PREVIEW={version:'0.6.0',counts:null,chronology:null,inspect:()=>({view:activeView,journey:journey?.inspect()||null,sourceVerified:[...verified],errors:[...errors],untranslated:[...E.unresolved],world:worldView?.inspect()||null,portraits:Object.keys(portraits).length}),loadArchive,preparePlay,setFrame};
document.addEventListener('click',e=>{const district=e.target.closest('[data-district]');if(district){location.hash='district/'+district.dataset.district;return;}const walk=e.target.closest('[data-walk]');if(walk){location.hash='walk/'+walk.dataset.walk;return;}const p=e.target.closest('[data-person]');if(p)openPerson(p.dataset.person);const site=e.target.closest('[data-site]');if(site)sendWalk({type:'terra:site',site:site.dataset.site});});
window.addEventListener('message',e=>{if(e.source!==$('courtyard-frame').contentWindow)return;if(e.data?.type==='courtyard-exit')location.hash='home';const mapping={capital:{name:'Níhmīwū',year:500,pid:294},bronze:{name:'Rikã̀žàval',year:-1500,pid:180},neolithic:{name:'Zirpuw',year:-6000,pid:443}};const district=Object.hasOwn(mapping,e.data?.district)?e.data.district:'capital';if(e.data?.type==='district-switch')location.hash='district/'+district;if(e.data?.type==='courtyard-archive'){const d=mapping[district];location.hash='atlas?year='+d.year+'&city='+encodeURIComponent(d.name)+'&polity='+d.pid;}});
...
journey=installTerraJourney({$,esc,fold,E,archive:()=>archive,frameIndex:()=>frameIndex,world:()=>worldView,active:()=>activeView,scenes:()=>SCENES,events:()=>translatedEvents,people:()=>people,setFrame,preparePlay,sendWalk,fail,walkState:()=>$('walk-frame').contentWindow?.GAME?.inspect?.()});
home();route();
})();
