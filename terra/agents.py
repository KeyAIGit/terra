...
def choose_actions(rng, co: Cohort, idx, ctx: dict, temperature: float = 0.55) -> np.ndarray:
    ...
    # Вклад в производственные действия центрирован по народу: пуганый
    # работает и копит БОЛЬШЕ соседЕЙ, но народ в целом не производит
    # больше, чем производил бы без памяти о голоде, — иначе каждый голод
    # разгоняет демографию, и к железному веку планета перегревается
    # (проверено A/B-прогоном: 896 млн к 500 н. э. вместо 178).
    c_hun = s_hun - float(s_hun.mean())
    U[:, AI["store"]] += 1.35 * c_hun
    U[:, AI["intensify"]] += 0.75 * c_hun
