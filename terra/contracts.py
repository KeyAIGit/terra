...
ORE_KEYS = (
    ...
    "silver",
    "coal",
    # ── недра, которые становятся ресурсом только в новое время ──
    # До поры это просто чёрная жижа, белый песок и тяжёлый камень: они лежат
    # в земле с начала мира, но нужны лишь тому, кто уже умеет их взять.
    "oil",        # нефть и битум — двигатель, химия, пластик
    "silica",     # чистый кварцевый песок — стекло, оптика, кремний
    "bauxite",    # глинозём — лёгкий металл, без него нет полёта
    "uranium",    # уран — сила, которая либо освещает, либо сжигает
)
...
AFFORDANCES = (
    ...
    # Не «технологии», а именно новые роды действия: впервые за всю историю
    # человек получает работу не из мышц, разделяет вещество на составные
    # части, считает без головы, говорит дальше, чем видит, и повторяет
    # деталь один в один.
    "power",      # работа не из мышц: вода, пар, электричество
    "refine",     # разделение вещества: перегонка, кокс, электролиз
    "compute",    # счёт без человека
    "transmit",   # сигнал дальше прямой видимости
    "precision",  # повторяемость детали, взаимозаменяемость
)
