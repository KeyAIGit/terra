...
class Sim:
    ...
    def _step_polity(self, poly: Polity, dt: int, clim, nb: dict, mats,
        ...
        # Порог 1.3 — чуть выше всего, что даёт домодерная гигиена вместе
        # (травы, повитухи, бани, мыло ≈ 1.18): потолок открывает только
        # микробная теория с вакциной и антибиотиками, иначе уже бронзовый
        # век растёт как двадцатый (проверено прогоном: 17 млн к −6000
        # против 7.9 у эталона).
        r_cap = 0.011 + 0.013 * float(np.clip((rep.effect("health") - 1.3) / 2.6, 0, 1))
    ...
    def _knowledge_step(self, poly, rep, idx, frac, mats, biomes, food_ratio, dt, K,
        ...
        if n_exp > 0 and mask.any():
            ...
            for _ in range(int(tries)):
                ...
                # которую он успел перенять. Веса откалиброваны прогоном A/B
                # против кода до «прожитой жизни»: средний взрослый искатель
                # даёт ~0.4 (как прежний навык beta(2,3)), великий мастер ~0.75,
                # юнец ~0.1. При старых весах (0.5/0.5/0.35) искатели насыщались
                # к 1.0 и мир перегревался: открытия на треть чаще, к −6000
                # населения на 33% больше, к 500 н. э. — 896 млн вместо 178.
                skill_eff = float(np.clip(
                    0.30 * co.mastery[who, ag.DI["craft"]]
                    + 0.30 * co.mastery[who].max()
                    + 0.20 * co.lore[who], 0, 1))
