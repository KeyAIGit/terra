...
await import('./courtyard/build.mjs');
const courtyardHTML=await fs.readFile(path.join(here,'courtyard/dist/Terra_Courtyard.html'),'utf8');
...
for(const [name,html]of Object.entries(raw))await fs.writeFile(path.join(site,'data',name+'.html.gz'),gzipSync(Buffer.from(html),{level:9}));
...
const release=JSON.parse(await fs.readFile(path.join(site,'release.json'),'utf8'));release.courtyard={asset_count:districtManifest.assets.length,runtime_bytes:districtManifest.runtimeBytes,playable:true,photorealism:'work-in-progress',simulation_changed:false};await fs.writeFile(path.join(site,'release.json'),JSON.stringify(release,null,2));
