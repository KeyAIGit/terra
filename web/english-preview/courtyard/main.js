...
import {attachQA} from './qa-hooks.js';
import {setupDistrictInterface} from './district-interface.js';
import {districtFrom,DISTRICTS} from './district-config.js';
import {buildNeighborhood} from './neighborhoods.js';
import {ResidentSimulation} from './resident-sim.js';
import {createResidentVisuals} from './resident-visuals.js';
import {createResidentUI} from './resident-ui.js';
...
const query=new URLSearchParams(location.search),hashQuery=new URLSearchParams(location.hash.slice(1));
const config=districtFrom(window.DISTRICT_BOOT||query.get('era')||hashQuery.get('era')||'capital');
let neighborhood=null,residentSimulation=null,residentVisuals=null,residentUI=null;
const testMode=query.get('qa')==='1';
const mobile=matchMedia('(pointer:coarse)').matches;
const audit={version:'0.6.0',ready:false,errors:[],assetsLoaded:[],walkableArea:config.id+'-inhabited-district',district:config.id,year:config.year,historyChanged:false,modelNames:[],photorealism:'work-in-progress'};
...
const keys=new Set(),velocity=new THREE.Vector2(),touchVector=new THREE.Vector2(),namedView=config.views;
function clearInput(){keys.clear();touchVector.set(0,0);drag=null;}
function blocked(px,pz){if(residentSimulation)return residentSimulation.nav.blocked(px,pz,.25,true);if(px<-13.2||px>13.2||pz<-12.15||pz>22.65)return true;for(const c of colliders){if(c.type==='circle'){if(Math.hypot(px-c.x,pz-c.z)<c.r+.24)return true;}else{const dx=px-c.x,dz=pz-c.z,cs=Math.cos(c.rot),sn=Math.sin(c.rot),lx=cs*dx-sn*dz,lz=sn*dx+cs*dz;if(Math.abs(lx)<c.w+.24&&Math.abs(lz)<c.d+.24)return true;}}return false;}
function teleport(name){const original=namedView[name];if(!original)return false;let p=null;
 for(const radius of [0,.7,1.25,1.8]){for(let i=0;i<(radius?12:1);i++){const angle=i/12*Math.PI*2,candidate=[original[0]+Math.cos(angle)*radius,original[1]+Math.sin(angle)*radius,original[2],original[3]];if(!blocked(candidate[0],candidate[1])&&!residentSimulation?.agents.some(a=>Math.hypot(a.x-candidate[0],a.z-candidate[1])<.6)){p=candidate;break;}}if(p)break;}
 if(!p)return false;[x,z,yaw,pitch]=p;$('places').value=name;velocity.set(0,0);clearInput();return true;}
...
$('archive').onclick=()=>parent!==window?parent.postMessage({type:'courtyard-archive',district:config.id},'*'):location.assign('../#atlas?year='+config.year+'&city='+encodeURIComponent(config.name)+'&polity='+config.pid);
...
function update(dt){const side=(keys.has('KeyD')||keys.has('ArrowRight')?1:0)-(keys.has('KeyA')||keys.has('ArrowLeft')?1:0)+touchVector.x,forward=(keys.has('KeyW')||keys.has('ArrowUp')?1:0)-(keys.has('KeyS')||keys.has('ArrowDown')?1:0)-touchVector.y;const len=Math.max(1,Math.hypot(side,forward)),speed=keys.has('ShiftLeft')?3.3:1.85;const vx=(Math.cos(yaw)*side-Math.sin(yaw)*forward)/len*speed,vz=(-Math.sin(yaw)*side-Math.cos(yaw)*forward)/len*speed;velocity.lerp(new THREE.Vector2(vx,vz),1-Math.exp(-12*dt));let nx=x+velocity.x*dt,nz=z+velocity.y*dt;const nearResident=(px,pz)=>residentSimulation?.agents.some(a=>Math.hypot(a.x-px,a.z-pz)<.46);if(!blocked(nx,z)&&!nearResident(nx,z))x=nx;if(!blocked(x,nz)&&!nearResident(x,nz))z=nz;const moving=velocity.length();camera.position.set(x,1.68+(moving>.15?Math.sin(time*8)*.014:0),z);camera.rotation.set(pitch,yaw,0,'YXZ');
 if(residentSimulation){residentSimulation.tick(dt,{x,z,y:1.65,yaw:yaw+Math.PI});residentVisuals?.update(dt,time,{x,z});}
...
function loop(ts){if(disposed)return;requestAnimationFrame(loop);const dt=Math.min(.04,(ts-last)/1000||.016);if(last&&loaded){frameMs.push(ts-last);if(frameMs.length>300)frameMs.shift();}last=ts;frames++;if(!paused){time+=dt;update(dt);}residentUI?.update();renderer.info.reset();if(composer)composer.render();else renderer.render(scene,camera);}
...
Object.assign(window.COURTYARD,{inspect:()=>({...audit,ready:loaded&&!disposed,player:{x,z,y:camera.position.y,yaw,pitch},paused,disposed,animationTime:time,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,frames,frameTimeMedian:frameMs.length?[...frameMs].sort((a,b)=>a-b)[Math.floor(frameMs.length*.5)]:null,frameTimeP95:frameMs.length?[...frameMs].sort((a,b)=>a-b)[Math.floor(frameMs.length*.95)]:null,collisionObjects:colliders.length,neighborhood:neighborhood?.info(),residents:residentSimulation?.inspect(),gait:residentVisuals?.inspect(),interaction:residentUI?.inspect()}),teleport,pause:setPause,dispose,blocked});
async function main(){const manifest=window.COURTYARD_MANIFEST||await(await fetch('ASSETS.json?v=0.6.0')).json();if(window.COURTYARD_MANIFEST){const link=document.querySelector('a[href="ASSETS.json"]');if(link)link.href=URL.createObjectURL(new Blob([JSON.stringify(manifest,null,2)],{type:'application/json'}));}await loadMaterials(manifest);if(disposed)return;const env=await new RGBELoader(manager).loadAsync(asset('assets/sky.hdr'));env.mapping=THREE.EquirectangularReflectionMapping;const pmrem=new THREE.PMREMGenerator(renderer);environmentTarget=pmrem.fromEquirectangular(env);scene.environment=environmentTarget.texture;scene.environmentIntensity=.47;scene.background=env;scene.backgroundIntensity=.92;scene.backgroundBlurriness=.02;textures.push(env);pmrem.dispose();await loadModels(manifest);if(disposed)return;neighborhood=buildNeighborhood({world,mats,box,add,worldUV,ownMat,placeModel,colliders,floor,fountain,market,archFill,archRing,movingCloth},config);neighborhood.populate();flush();residentSimulation=new ResidentSimulation(neighborhood);residentVisuals=await createResidentVisuals(world,residentSimulation,mats,asset);residentUI=createResidentUI(residentSimulation,camera,()=>({x,z,y:1.65,yaw:yaw+Math.PI}),setPause,()=>paused);attachQA(window.COURTYARD,neighborhood,residentSimulation,residentVisuals,(px,pz,heading)=>{x=px;z=pz;yaw=heading;velocity.set(0,0);});audit.assetsLoaded.push(manifest.assets.find(a=>a.type==='hdri')?.id||'HDR environment');setupPost();setQuality('auto');if(!teleport('entrance'))throw Error('District spawn is blocked');resize();renderer.compile(scene,camera);await renderer.compileAsync(scene,camera);update(0);renderer.info.reset();composer.render();loaded=true;audit.ready=true;window.COURTYARD.ready=true;$('loading').hidden=true;requestAnimationFrame(loop);}
setupDistrictInterface(config,id=>{
 if(parent!==window)parent.postMessage({type:'district-switch',district:id},'*');
 else if(window.COURTYARD_ASSETS){location.hash='era='+id;location.reload();}
 else location.href=location.pathname+'?era='+id;
},setPause);
